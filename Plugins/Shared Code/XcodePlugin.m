...
BOOL XcodePluginShouldLoad(BOOL _LoadInXcodeBuild)
{
    ...
    if (!_LoadInXcodeBuild)
    {
        ...
        XcodePluginConfirmOrPerform([processName caseInsensitiveCompare: @"xcode"] == NSOrderedSame, return NO);
    }
    ...
    NSArray *versionComponents = [[[NSBundle mainBundle] objectForInfoDictionaryKey: @"CFBundleShortVersionString"] componentsSeparatedByString: @"."];
        XcodePluginConfirmOrPerform(versionComponents && [versionComponents count], return NO);
    ...
    XcodePluginConfirmOrPerform(xcodeMajorVersion == 13, return NO);
    XcodePluginConfirmOrPerform(xcodeMinorVersion == 1, return NO);
...
const NSUInteger XcodePluginMaxLoadAttempts = 3;
IMP XcodePluginOverrideMethod(Class class0, SEL selector, IMP newImplementation)
{
    ...
    classMethods = class_copyMethodList(class0, &classMethodsCount);
        XcodePluginAssertOrPerform(classMethods, goto cleanup);
    ...
    else
    {
        ...
        const char *types = method_getTypeEncoding(originalMethod);
            XcodePluginAssertOrPerform(types, goto cleanup);
        
        BOOL addMethodResult = class_addMethod(class0, selector, newImplementation, types);
            XcodePluginAssertOrPerform(addMethodResult, goto cleanup);
    }
...
void XcodePluginTraceViewHierarchy(NSView* _pView, int _Depth)
{
  ...
  for (NSView * pView in [_pView subviews])
      XcodePluginTraceViewHierarchy(pView, _Depth + 1);
}
